...
- **Provider seam — `src/mcp/data.ts`** is the *only* file that knows more than one fantasy backend exists. `FANTASY_PROVIDER=yahoo|sleeper|fixture` picks `yahoo-data.ts`, `sleeper-data.ts` or `fixture-data.ts` at startup; the re-export is typed `typeof yahoo`, so the Yahoo module's signatures ARE the provider contract and the compiler proves the others implement it. Everything above this file (tools, digest, evals) must stay provider-blind — never import a `*-data` module directly from consumers. `fixture` is the human-in-the-loop degraded mode (MUFF-50/58): it serves hand-transcribed weeks from the store at `fixtures/weekly/<season>-wNN.json` (no "latest" pointer — the newest key is the latest week) and refuses `get_roster`/`get_matchup`. `data.ts` also exports `getProvenance(week)`; every `WeekFacts` carries `provenance` (`source: yahoo|sleeper|manual|synthetic`, plus who/when for manual), which is archived and logged but stripped before the model sees the facts.
